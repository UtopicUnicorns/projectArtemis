<?php
  ...
  foreach($guildsWeShare as $guild) {
    ...
    if(!$guildCheck->message == 'Missing Access') {
      $commonGuilds .= '<tr>
                          <td style="border-radius: 0px 50px; width: 5vw; height: 5vw; background-size: contain; background-repeat: no-repeat; background-position: center; background-image: url(\'https://cdn.discordapp.com/icons/' . $guild->id . '/' . $guild->icon . '.png?size=256\')"></td>
                          <td><h1 style="margin-left: 1vw; color: rgba(0, 0, 0, 0.3); font-size: 1vw;" id="nameInUserPanel">'.$guild->name.'</h1></td>
                        </tr>'; 
    }
  }
  foreach($guildsWeShare as $guildIAmIn) {
    $myGuilds .= '<tr>
                    <td style="border-radius: 0px 50px; width: 5vw; height: 5vw; background-size: contain; background-repeat: no-repeat; background-position: center; background-image: url(\'https://cdn.discordapp.com/icons/' . $guildIAmIn->id . '/' . $guildIAmIn->icon . '.png?size=256\')"></td>
                    <td><h1 style="margin-left: 1vw; color: rgba(0, 0, 0, 0.3); font-size: 1vw;" id="nameInUserPanel">'.$guildIAmIn->name.'</h1></td>
                  </tr>'; 
                }
