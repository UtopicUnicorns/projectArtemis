...
module.exports = {
  appMake: {
    appName: 'stop', 
    appDescription: 'Stop the music player.', 
    type: 'chatInput', 
    permissions: [] 
  },
  name: 'stop', 
  ...
  async execute(msg)  {
    ...
    if(queueGet !== 'make a connection first') {
      if(queueGet.channel !== resChannel) return failureYouAre(`I am already playing music in <#${queueGet.channel}>, move there to stop the player.`);
      interaction.create( { 
        id: msg.id, 
        token: msg.token, 
        type: 'channelMessage', 
        ...
        .catch((err) => console.log(err));
      channel.msgSend({ 
        content: `👤 ${msg.member.user.username}#${msg.member.user.discriminator} stopped the player.`, 
