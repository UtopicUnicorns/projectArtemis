...
module.exports = {
  appMake: {
    appName: 'volume', 
    appDescription: 'Set the bot volume.', 
    type: 'chatInput', 
    permissions: [],
    build: [{
      commandName: 'set',
      commandDescription: 'A value between 1 and 100.',
      commandType: 4,
  ...
  async execute(msg)  {
    ...
    if(queueGet !== 'make a connection first') {
      if(queueGet.channel !== resChannel) return failureYouAre(`I am already playing music in <#${queueGet.channel}>, move there to adjust the volume.`);
      interaction.create( { 
        id: msg.id, 
        token: msg.token, 
        type: 'channelMessage', 
        ...
        .catch((err) => console.log(err));
      return voice.volume({
        command: msg.data.options[0].value, 
      ...
      .then((val) =>  {
        channel.msgSend({ 
          content: `👤 ${msg.member.user.username}#${msg.member.user.discriminator} 🔊 ${queueGet.settings.volume * 100}%`, 
          channel: queueGet.channel 
        })
          .catch((err) => console.log(err));
      })
    ...
    return interaction.create({ id: msg.id, token: msg.token, type: 'channelMessage', data: { content: `There is no music playing.`, visibleForAll: true } }).catch((err) => console.log(err));
  },
};
