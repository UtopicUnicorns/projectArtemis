...
module.exports =  {
  appMake:  {
    appName: 'pitch', 
    appDescription: 'Change music pitch.', 
    type: 'chatInput', 
    permissions: [],
    build: [{
      commandName: 'set',
      commandDescription: 'Pitch higher, lower or normal.',
      commandType: 3,
  ...
  async execute(msg)  {
    ...
    if(queueGet !== 'make a connection first') {
      if(queueGet.channel !== resChannel) return failureYouAre(`I am already playing music in <#${queueGet.channel}>, move there to adjust the pitch.`);
      interaction.create({
        id: msg.id, 
        token: msg.token, 
        type: 'channelMessage', 
      ...
      return voice.pitch({
        ...
        .then((val) =>  {
          channel.msgSend({ content: `👤 ${msg.member.user.username}#${msg.member.user.discriminator} adjusted the pitch.`, channel: queueGet.channel }).catch((err) => console.log(err));
        })
        .catch((err) => console.log(err));
    }
