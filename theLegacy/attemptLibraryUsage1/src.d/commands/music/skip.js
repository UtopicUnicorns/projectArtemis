...
module.exports =  {
  appMake:  {
    appName: 'skip', 
    appDescription: 'Skip the current playing song.',
    type: 'chatInput',
    permissions: [] 
  },
  name: 'skip',
  ...
  async execute(msg) {
    ...
    if(queueGet !== 'make a connection first') {
      if(queueGet.channel !== resChannel) return failureYouAre(`I am already playing music in <#${queueGet.channel}>, move there to skip the song.`);
      interaction.create({
        id: msg.id,
        token: msg.token,
        type: 'channelMessage',
        ...
        .catch((err) => console.log(err));
      channel.msgSend({
        content: `👤 ${msg.member.user.username}#${msg.member.user.discriminator} skipped the current song.`,
