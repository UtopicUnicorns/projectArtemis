...
module.exports =  {
  appMake:  {
    appName: 'rolemenudelete', 
    appDescription: 'Spawn rolemenu for deletion.', 
    type: 'chatInput', 
    permissions: [],
    build: [{
      commandName: 'slot',
      commandDescription: 'Spawn a rolemenu to delete roles from the list.',
      commandType: 3,
  ...
  async execute(msg, sql) {
    ...
    let componentObject = new component()
      .menu({ custom_id: `${msg.guild_id}:roleMenuDelete:${slotNum}`,  
              place_holder: 'Select role for deletion', 
              min_val: 1, 
              max_val: 1 });
